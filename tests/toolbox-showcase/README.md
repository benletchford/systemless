...
Every Systemless 68K image in the comparison tables now shows the **4× desktop
presentation surface**, captured during the complete interaction sequence. Open an
image to inspect its full 3200×2400 resolution. PowerPC currently uses the logical
raster; its separate table shows that supported output. The 800×600 guest baselines
...
The 68K review images retain native outline detail; the oracle images retain their
original resolution.
...
### PowerPC logical presentation

PowerPC currently uses the logical raster, including with the new bundled fonts.
These 16-bit captures do **not** have the 68K desktop's 4× outline layer.
