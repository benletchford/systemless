...
fn license_html(entry: &CompiledEntry) -> Result<String> {
    ...
    write!(html, "<p>Screenshots, plugins and other files may have different terms. See their individual records in the <a href=\"{}\" target=\"_blank\" rel=\"noopener noreferrer\">entry source</a>.</p></section>", escape(&entry.community.source))?;
    Ok(html)
}
