...
const PPC_CUR_RES_FILE_ADDR: u32 = 0x0000_0A5A;
...
const PPC_THE_ZONE_ADDR: u32 = 0x0000_0118;
...
const PPC_CLOSED_RESOURCE_REF_NUM: i16 = i16::MIN;
...
const PPC_RES_PROTECTED_ATTR: u16 = 0x0008;
...
const PPC_MAIN_CTABLE_SIZE: u32 = 8 + 256 * 8;
...
fn ppc_interrupt_callback_stack_pointer(interrupted_sp: u32) -> u32 {
...
impl PpcLoadedApp {
    ...
    #[cfg(test)]
    fn set_heap_limit(&self, heap_limit: u32) {
        let mut memory_manager = self.process_memory_manager.0.borrow_mut();
        if memory_manager.has_native_allocator() {
            memory_manager.mutate_native_allocator(|allocator| {
    ...
    fn run_with_hle_imports_with_trace(
        &mut self,
        max_cycles: u64,
        trace_imports: bool,
        trace_fetches: bool,
...
pub(super) fn ppc_serialized_resource_fork(
...
fn map_instantiated_sections(data: &[u8]) -> Result<Vec<MappedSection>, PpcLoadError> {
...
fn dispatcher_target_for_import(
    library_name: &str,
    symbol_name: &str,
...
#[allow(clippy::too_many_arguments)]
pub(super) fn ppc_dispose_process_native_handle(
...
#[allow(clippy::too_many_arguments)]
pub(super) fn ppc_apply_process_native_resource_handle(
    memory_manager: &ProcessNativeMemoryManager,
    memory: &mut PpcSectionMem,
    heap_cursor: &mut u32,
    last_mem_error: &mut i16,
...
fn ppc_zero_guest_bytes(memory: &mut PpcSectionMem, addr: u32, len: u32) -> bool {
...
fn ppc_enqueue_compatibility(memory: &mut PpcSectionMem, element: u32, header: u32) -> bool {
    if element == 0
        || header == 0
...
fn ppc_dequeue_compatibility(memory: &mut PpcSectionMem, element: u32, header: u32) -> i16 {
...
fn ppc_rgb555_to_rgb16(pixel: u16) -> [u16; 3] {
...
pub(super) fn ppc_forget_handle_state(handle: u32, handle_states: &mut Vec<PpcHandleStateRecord>) {
    handle_states.retain(|record| record.handle != handle);
}

fn ppc_alloc_handle_with_bytes(
    memory: &mut PpcSectionMem,
    heap_cursor: &mut u32,
    heap_limit: u32,
    handles: &mut Vec<PpcHandleRecord>,
...
fn ppc_alloc_recyclable_handle(
    memory: &mut PpcSectionMem,
    heap_cursor: &mut u32,
    heap_limit: u32,
    handles: &mut Vec<PpcHandleRecord>,
...
#[allow(clippy::too_many_arguments)]
pub(super) fn ppc_read_packed_bitmap_index(
    memory: &mut PpcSectionMem,
    base_addr: u32,
    row_bytes: u32,
    pixel_size: u16,
...
fn ppc_current_gdevice_record(memory: &mut PpcSectionMem, current_gdevice: u32) -> Option<u32> {
    memory
...
fn ppc_inverse_table_bytes(colors: &[(u8, [u16; 3])], resolution: u16) -> Vec<u8> {
...
#[allow(clippy::too_many_arguments)]
pub(super) fn ppc_process_alloc_handle(
    memory_manager: &mut ProcessNativeMemoryManager,
    memory: &mut PpcSectionMem,
    heap_cursor: &mut u32,
    last_mem_error: &mut i16,
...
#[allow(clippy::too_many_arguments)]
pub(super) fn ppc_allocator_view_dispose_handle(
    allocator: Option<&mut PpcProcessAllocatorView<'_>>,
    memory: &mut PpcSectionMem,
    heap_cursor: &mut u32,
    heap_limit: u32,
...
pub(super) fn ppc_aligned_heap_allocation_bounds(
    memory: &PpcSectionMem,
    heap_cursor: u32,
    heap_limit: u32,
    size: u32,
...
pub(super) fn ppc_update_zone_free_bytes(memory: &mut PpcSectionMem, heap_cursor: u32, heap_limit: u32) {
...
pub(super) fn ppc_optional_pstring_output_can_write(
    memory: &mut PpcSectionMem,
    addr: u32,
    bytes: &[u8],
) -> bool {
...
fn ppc_allocation_size(size: u32) -> Option<u32> {
...
fn align_up(value: u32, align: u32) -> Result<u32, PpcLoadError> {
    if align <= 1 {
        return Ok(value);
    }
    let mask = align - 1;
